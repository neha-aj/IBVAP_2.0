...
of event-alert-service's/reid-service's own groups on the same stream
(SAS §11: multiple consumer groups per stream is the supported pattern for
this) -- and runs each event through this camera's `PPEService`."""
...
_TRACK_EVENTS_PROCESSED = Counter(
    "ppe_track_events_processed_total", "Track lifecycle events run through PPE Detection", ["camera_id"]
)
_VIOLATIONS_REPORTED = Counter(
    "ppe_violations_reported_total", "PPE violations reported", ["camera_id"]
)
...
class TrackConsumer:
    ...
    def start(self) -> None:
        self._stop_requested = False
        self._task = asyncio.create_task(self._run(), name=f"ppe-consumer-{self.camera_id}")
    ...
    async def _run(self) -> None:
        ...
        while not self._stop_requested:
            try:
                ...
                logger.warning("ppe_stream_read_failed", camera_id=self.camera_id, error=str(exc))
        ...
        try:
            ...
            if raw:
                ...
                with correlation_id_context(track_event.correlation_id):
                    ...
                    missing = await self._ppe_service.process_track_event(track_event)
                    if missing is not None:
                        _VIOLATIONS_REPORTED.labels(camera_id=self.camera_id).inc()
        except Exception as exc:
            logger.warning("ppe_processing_failed", camera_id=self.camera_id, error=str(exc))
        finally:
