...
from app.inference.model_registry import build_engine
...
async def lifespan(app: FastAPI):
    ...
    manager = ConsumerManager(settings, engine)
    app.state.consumer_manager = manager
    await manager.start()
...
def create_app() -> FastAPI:
    app = FastAPI(title="IBVAP Detection Service", version="0.1.0", lifespan=lifespan)
