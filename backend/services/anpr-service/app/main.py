...
from app.api import anpr as anpr_routes
...
async def lifespan(app: FastAPI):
    ...
    yield
    await manager.stop()
    logger.info("anpr_service_stopped")


def create_app() -> FastAPI:
    app = FastAPI(title="IBVAP ANPR Service", version="0.1.0", lifespan=lifespan)
    install_correlation_id_middleware(app)
    install_metrics(app, settings.service_name)
    install_error_handlers(app)
    app.include_router(anpr_routes.router)
