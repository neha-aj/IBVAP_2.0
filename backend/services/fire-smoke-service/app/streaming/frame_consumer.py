...
to `settings.sample_interval_seconds` (doc09 §2.6: "2-5fps is standard...
full detection-service framerate isn't necessary and would waste GPU
budget" -- no GPU here, but the same reasoning applies to CPU heuristic
cost), and runs each sampled frame through `FireSmokeService`.
"""
...
_FRAMES_SAMPLED = Counter(
    "fire_smoke_frames_sampled_total", "Frames actually run through the fire/smoke heuristics", ["camera_id"]
)
_DETECTIONS_REPORTED = Counter(
    "fire_smoke_detections_reported_total", "Fire/smoke detections reported", ["camera_id", "event_type"]
)
...
class FrameConsumer:
    def __init__(
        ...
        *,
        ...
        fire_smoke_service: FireSmokeService,
        group_name: str,
        read_count: int,
        block_ms: int,
        ...
        self._service = fire_smoke_service
    ...
    def start(self) -> None:
        self._stop_requested = False
        self._task = asyncio.create_task(self._run(), name=f"fire-smoke-consumer-{self.camera_id}")
    ...
    async def _run(self) -> None:
        ...
        while not self._stop_requested:
            try:
                ...
                logger.warning("fire_smoke_stream_read_failed", camera_id=self.camera_id, error=str(exc))
    ...
    async def _process_message(self, message_id: bytes, fields: dict[bytes, bytes]) -> None:
        try:
            ...
            with correlation_id_context(correlation_id):
                ...
                event_type = await self._service.process_frame(self.camera_id, frame)
                if event_type is not None:
                    _DETECTIONS_REPORTED.labels(camera_id=self.camera_id, event_type=event_type).inc()
        except Exception as exc:
            logger.warning("fire_smoke_processing_failed", camera_id=self.camera_id, error=str(exc))
        finally:
