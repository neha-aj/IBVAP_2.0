...
async def lifespan(app: FastAPI):
    manager = ReconcileManager(settings)
    app.state.reconcile_manager = manager
    await manager.start()
    logger.info("fire_smoke_service_started")
    yield
    await manager.stop()
    logger.info("fire_smoke_service_stopped")


def create_app() -> FastAPI:
    app = FastAPI(title="IBVAP Fire & Smoke Detection Service", version="0.1.0", lifespan=lifespan)
