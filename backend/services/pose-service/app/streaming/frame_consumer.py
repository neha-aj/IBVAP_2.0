...
of detection-service's, fire-smoke-service's, and every other pipeline
stage's own group on the same stream (SAS §11: multiple consumer groups
per stream is the supported pattern for this) -- throttles to
`settings.sample_interval_seconds` (slower than fire-smoke-service's
default: see `app/core/config.py`'s own comment on why real pose-model
inference costs more per frame than a color heuristic), and runs each
sampled frame through `PoseService`. Structurally identical to
fire-smoke-service's own `frame_consumer.py`."""
...
_FRAMES_SAMPLED = Counter(
    "pose_frames_sampled_total", "Frames actually run through the pose classifier", ["camera_id"]
)
_POSES_PUBLISHED = Counter(
    "pose_readings_published_total", "Posture readings published", ["camera_id", "posture"]
)
...
class FrameConsumer:
    ...
    def start(self) -> None:
        self._stop_requested = False
        self._task = asyncio.create_task(self._run(), name=f"pose-consumer-{self.camera_id}")
    ...
    async def _run(self) -> None:
        ...
        while not self._stop_requested:
            try:
                ...
                logger.warning("pose_stream_read_failed", camera_id=self.camera_id, error=str(exc))
    ...
    async def _process_message(self, message_id: bytes, fields: dict[bytes, bytes]) -> None:
        try:
            ...
            with correlation_id_context(correlation_id):
                ...
                readings = await self._service.process_frame(self.camera_id, frame)
                for reading in readings:
                    _POSES_PUBLISHED.labels(camera_id=self.camera_id, posture=reading.posture).inc()
        except Exception as exc:
            logger.warning("pose_processing_failed", camera_id=self.camera_id, error=str(exc))
        finally:
