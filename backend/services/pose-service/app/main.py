...
async def lifespan(app: FastAPI):
    ...
    await manager.start()
    logger.info("pose_service_started")
    yield
    await manager.stop()
    logger.info("pose_service_stopped")


def create_app() -> FastAPI:
    app = FastAPI(title="IBVAP Pose (Posture) Detection Service", version="0.1.0", lifespan=lifespan)
