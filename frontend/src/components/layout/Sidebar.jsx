...
export default function Sidebar({open=false,onClose=()=>{}}){
  ...
  // "Cameras" (add/edit/delete) is admin-only -- a non-admin still gets
  // "Live Surveillance" (viewing existing feeds) and everything else.
  const groups=[{label:'Overview',items:[['Dashboard','/',LayoutDashboard]]},{label:'Surveillance',items:[['Live Surveillance','/surveillance',MonitorPlay],...(isAdmin?[['Cameras','/cameras',Video]]:[]),['Alerts','/alerts',BellRing,activeAlertCount||undefined],['Events','/events',History]]},{label:'Investigation',items:[['Evidence','/evidence',FileVideo],['License Plates','/anpr',CarFront],['Person & Vehicle Search','/reid',UserRound]]},{label:'Analytics',items:[['Analytics','/analytics',ChartNoAxesCombined]]},{label:'System',items:[['Settings','/settings',Settings]]}];
